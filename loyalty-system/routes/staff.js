...
router.post('/register', authenticateToken, async (req, res) => {
    try {
        ...
        const staff = new Staff({ companyId: validCompanyId, name, email, password: hashedPassword });
        await staff.save();
...
router.post('/login', async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: staff._id, companyId: staff.companyId, userType: 'staff' }, process.env.JWT_SECRET, { expiresIn: '1h' });
        res.send({ token });
    } catch (error) {
        res.status(400).send(error);
    }
